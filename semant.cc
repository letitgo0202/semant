...
Symbol lookup_object(Symbol name,
  ...
  if (!exclude_current_scope) {                     //排除当前作用域
    prevdef = ENVIRONMENT[c_name].object_table->lookup(name);
  }

  if (current_scope_only) {                         //只考虑当前作用域,不考虑父类
...
bool ClassTable::class_defined(Symbol class_name){
    return((child_to_parent.find(class_name)!=child_to_parent.end())||class_name==SELF_TYPE);
}
...
Symbol find_common_ancestor(Symbol s1,  Symbol s2, Class_ c){
    ...
    p1=classtable->get_path(s1);
    p2=classtable->get_path(s2);
...
ClassTable::ClassTable(Classes classes) : semant_errors(0) , error_stream(cerr) {
    ...
    for(int i=classes->first();classes->more(i);i=classes->next(i)){
      ...
      if(class_defined(curr->get_name())){
        semant_error(curr)<<"redefinition of class" <<curr->get_name()->get_string()<<"\n";       
      }

      add_pair(curr->get_name(),curr->get_parent());
    ...
    for(it=parents.begin();it!=parents.end();it++){
      Symbol name = *it;
      if(name==Int||name==Bool||name==SELF_TYPE||name==Str){
        semant_error(curr)<<"could not inherit basic class" <<name->get_string()<<"\n";  
      }

      if(!class_defined(name)){
        semant_error(curr)<<"inherit from undefined class" <<name->get_string()<<"\n";
      }
    }
    ...
    for(int i=classes->first();classes->more(i);i=classes->next(i)){
      ...
      if(is_cyclic(curr->get_name())){
        semant_error(curr)<<"inherit cycle (" <<curr->get_name()->get_string()<<").\n";
      }
    }
}
...
void ClassTable::install_basic_classes() {
  ...
  Class_ basic_classes[]={Object_class,Str_class,IO_class,Bool_class,Int_class};
  for(int i=0;i<5;i++){
    add_pair(basic_classes[i]->get_name(),basic_classes[i]->get_parent());
...
Symbol assign_class::get_expr_type(Class_ c){
  if(get_name()==self){
    semantic_error(c->get_filename(),this)<<"Can not assgin to 'self'\n";
  }
  Symbol id_type=lookup_object(get_name(),c);

  if(!id_type){
    semantic_error(c->get_filename(),this)<<"Assignment to undeclared varible"<<get_name()->get_string()<<".\n";
    type = Object;
    return type;
  }
  ...
  if(!types_compatible(e_type,id_type,c)){
    semantic_error(c->get_filename(),this)<<"inferred type"<<e_type->get_string()<<"of initialization of"<<get_name()->get_string()<<"doesn't conform to declared type"<<id_type->get_string()<<".\n";
    type = Object;
    return type;
  }
  type = e_type;
...
Symbol static_dispatch_class::get_expr_type(Class_ c){
  ...
  if(!class_defined(get_type())){
    semantic_error(c->get_filename(),this)<<"Static dispatch to undefined class"<<get_type()->get_string()<<".\n";
    type = Object;
    return type;
  }

  if(!types_compatible(e0_type,get_type(),c)){
    semantic_error(c->get_filename(),this)<<"Expression type"<<e0_type->get_string()<<"does not conform to declared static dispatch"<<get_type()->get_string()<<".\n";
    type = Object;
    return type;
  }
  ...
  if(!method){
    semantic_error(c->get_filename(),this)<<"Dispatch to undefined method"<<get_method_name()->get_string()<<".\n";
    type = Object;
    return type;
  }
...
Symbol loop_class::get_expr_type(Class_ c){
  Symbol pred,body;
  pred=get_pred()->get_expr_type(c);
  body=get_body()->get_expr_type(c);

  if(pred != Bool){
    semantic_error(c->get_filename(),this)<<"Loop condition is not Bool type.\n";
  }
  type = Object;
  return type; 
}
...
Symbol typcase_class::get_expr_type(Class_ c){
  ...
  for(int i=cases->first();cases->more(i);i=cases->next(i)){
    ...
    if(duplicates.find(ca->get_type_decl())!=duplicates.end()){
      semantic_error(c->get_filename(),this)<<"Duplicate branch"<<ca->get_type_decl()->get_string()<<"in case statement.\n";
    }
...
Symbol typcase_class::process_case(Case c,Class_ cl){
  ...
  if(c->get_type_decl()==SELF_TYPE){
    semantic_error(cl)<<"Identifier "<<c->get_name()->get_string()<<" declared with type SELF_TYPE in case branch.\n";
  }
...
Symbol let_class::get_expr_type(Class_ c){
  if(get_id()==self){
    semantic_error(c->get_filename(),this)<<"'self' is not bound in a 'let' expression.\n";
  }

  if(!class_defined(get_type_decl())){
    semantic_error(c->get_filename(),this)<<"Class "<<get_type_decl()->get_string()<<"of let-bound indentifier "<<get_id()->get_string()<<"is not defined.\n";
  }
  Symbol init_type = get_init()->get_expr_type(c);
  if(init_type!=No_type){
    if(!types_compatible(init_type,get_type_decl(),c)){
      semantic_error(c->get_filename(),this)<<"Inferred type "<<init_type->get_string()<<"of initialization of "<<get_id()->get_string()<<"does not conform to indentifier's type "<<get_type_decl()->get_string()<<".\n";
    }
  }

  enterscope(c->get_name());
  add_object(get_id(),get_type_decl(),c);
...
Symbol plus_class::get_expr_type(Class_ c){
  ...
  if(!(e1==Int && e2==Int)){
    semantic_error(c->get_filename(),this)<<"Non-int arguments: "<<e1->get_string()<<"+"<<e2->get_string()<<".\n";
    type = Object;
  }
  return type;
}
...
Symbol neg_class::get_expr_type(Class_ c){
  ...
  if(type!=Int){
    semantic_error(c->get_filename(),this)<<"Non-Int arguments: "<<type->get_string()<<".\n";
    type = Object;
  }
  return type;
}
...
Symbol eq_class::get_expr_type(Class_ c){
  ...
  e1=get_e1()->get_expr_type(c);
  e2=get_e2()->get_expr_type(c);
  if(e1==Int||e1==Str||e1==Bool||e2==Int||e2==Str||e2==Bool){
    if(e1!=e2){
      semantic_error(c->get_filename(),this)<<"Illegal comparsion with a basic type.\n";
    }
  }
  type = Bool;
  return type;
...
Symbol comp_class::get_expr_type(Class_ c){
  type=get_expression()->get_expr_type(c);
  if(type!=Bool){
    semantic_error(c->get_filename(),this)<<"Argument is type "<<type->get_string()<<" instead of Bool.\n";
    type = Object;
  }
  return type;
}
...
Symbol new__class::get_expr_type(Class_ c){
  ...
  if(!class_defined(type)){
    semantic_error(c->get_filename(),this)<<"'new' used with undefined type "<<type->get_string()<<".\n";
    type = Object;
  }
  return type;
}
...
Symbol object_class::get_expr_type(Class_ c){
  ...
  if(!type){
    semantic_error(c->get_filename(),this)<<"Undeclared identifier "<<get_name()->get_string()<<".\n";
    type = Object;
  }
  return type;
}
...
void method_class::add_feature(Class_ c){
  ...
  if(prevdef!=NULL){
    semantic_error(c->get_filename(),this)<<"Redefininition of method "<<get_method_name()->get_string()<<".\n";
  }
  if(!class_defined(method->return_type)){
    semantic_error(c->get_filename(),this)<<"Return type "<<get_return_type()->get_string()<<" of method "<<get_method_name()->get_string()<<" is not defined.\n";
  }

  for(int i=formals->first();formals->more(i);i=formals->next(i)){
    FormalPair p(formals->nth(i)->get_name(),formals->nth(i)->get_type());
    if(names_defined.find(p.name)!=names_defined.end()){
      semantic_error(c->get_filename(),this)<<"Formal parameter "<<p.name->get_string()<<" is multiply defined!\n";
    }
    names_defined.insert(p.name);
    if(p.name==self){
      semantic_error(c->get_filename(),this)<<"'self' can't be a formal parameter name.\n";
    }
    if(p.type==SELF_TYPE){
      semantic_error(c->get_filename(),this)<<"Formal parameter "<<p.name->get_string()<<" can't have type SELF_TYPE.\n";
    }
    if(!class_defined(p.type)){
      semantic_error(c->get_filename(),this)<<"Formal parameter "<<p.name->get_string()<<"has undefined type "<<p.type->get_string()<<".\n";
    }
    method->add_parameter(p);
  } 
  add_method(get_method_name(),method,c);
}
...
void method_class::check_inheritance_features(Class_ c){
  ...
  if(prevdef){
    if(prevdef->return_type!=get_return_type()){
      semantic_error(c->get_filename(),this)<<"In redefined method "<<get_method_name()->get_string()<<",the return type "<<get_return_type()->get_string()<<" is inconsistent with the orginal type "<<prevdef->return_type->get_string()<<".\n"; 
    }
...
void method_class::set_expression(Class_ c){
  ...
  enterscope(c->get_name());
  for(int i=0;i<method->number_of_params();i++){
    add_object(method->parameters[i].name,method->parameters[i].type,c);
  }
  Symbol type = get_expression()->get_expr_type(c);
  if(!types_compatible(type,method->return_type,c)){
    semantic_error(c->get_filename(),this)<<"Inferred return type "<<type->get_string()<<" of method "<<get_method_name()->get_string()<<" does not conform to declared type "<<method->return_type->get_string()<<".\n";
  }
  exitscope(c->get_name());
}
...
void attr_class::add_feature(Class_ c){
  ...
  if(get_attr_name()==self){
    semantic_error(c->get_filename(),this)<<"'self' can't be name of attribute.\n";
  }
  if(prevdef){
    semantic_error(c->get_filename(),this)<<"Redefiniation attribute "<<get_attr_name()->get_string()<<"./n";
  }
  if(!class_defined(get_type_decl()) && !type_reserved(get_type_decl())){
    semantic_error(c->get_filename(),this)<<"Type "<<get_type_decl()->get_string()<<" of an attribute "<<get_attr_name()->get_string()<<"is not defined.\n";
  }
...
void attr_class::check_inheritance_features(Class_ c){
  ...
  if(prevdef){
    semantic_error(c->get_filename(),this)<<"Redefined inherited attribute "<<get_attr_name()->get_string()<<" from ancestor class.\n";
  }
}
...
void attr_class::set_expression(Class_ c){
  ...
  if(!types_compatible(type,get_type_decl(),c)){
    semantic_error(c->get_filename(),this)<<"Inferred type "<<type->get_string()<<" of initialization attribute "<<get_attr_name()->get_string()<<" does not confirm to declared type "<<get_type_decl()->get_string()<<".\n";
  }
}
